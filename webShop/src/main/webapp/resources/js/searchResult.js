function initSeaResult(searchInfo){
	var pageCount = 1;
	if (searchInfo.productMaxCount == 0) {
		$("div#sr_wares").css("display", "none");
		$("div#sr_paging").css("display", "none");
		$("div#sr_failed").css("display", "block");
	} else {
		if (Number(eval(searchInfo.productMaxCount % 40)) == 0) {
			pageCount = parseInt(eval(searchInfo.productMaxCount / 40));
		} else {
			pageCount = parseInt(eval(searchInfo.productMaxCount / 40) + 1);
		}
		//添加查询结果商品信息
		for (var i = 0; i < searchInfo.productList.length; i++) {
			var product_id = searchInfo.productList[i].id;
			var product_name = searchInfo.productList[i].productName;
			var product_price = searchInfo.productList[i].shopPrice;
			var product_img =  ctxImg +searchInfo.productList[i].image;
			var wares_name = product_name + "<br>¥<b>" + product_price + "</b>";
			var htmlText = "<div class='col-sm-3 col-xs-3 a3 bd fade'><a href='' product_id='" + product_id +
				"'><img src='" + product_img +
				"' class='waresImg mgImg hvImg'><br><br><p class='textFont'>" + wares_name + "</p></a></div>";
			var $new = $(htmlText);
			$("div#sr_wares").append($new);
		}
		//设置翻页数量
	...
	$("ul.pagination").on("click", "li", function() {
		...
		if ($(this).hasClass("prev")) {
			if ($(this).next().text() == nowActivePage.text()) {
				...
				/*提交翻页获取数据*/
				var product_text = {
					"offset": Number(prevPage-1),
					"limit": 40,
					"searchText": keyword
				};
				var search_url = ctx+"/search/" + keyword + "/" + Number(prevPage-1) + "/40";
				$.ajax({
					url: search_url,
					type: "get",
			...
			if ($(this).prev().text() == nowActivePage.text()) {
				...
				/*提交翻页获取数据*/
				var product_text = {
					"offset": Number(nextPage-1),
					"limit": 40,
					"searchText": keyword
				};
				var search_url = ctx+"/search/" + keyword + "/" + Number(nextPage-1) + "/40";
				$.ajax({
					url: search_url,
					type: "get",
					//contentType: "application/json; charset=utf-8",
			...
			/*提交翻页获取数据*/
			var product_text = {
				"offset": Number(nowClickPage-1),
				"limit": 40,
				"searchText": keyword
			};
			var search_url = ctx+"/search/" + keyword + "/" + Number(nowClickPage-1) + "/40";
			$.ajax({
				url: search_url,
				type: "get",
				//contentType: "application/json; charset=utf-8",
